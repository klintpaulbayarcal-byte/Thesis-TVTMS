...
try {
    ...
    if (-not (Test-Port 5000)) {
        $logDirectory = Join-Path $Backend 'logs'
        if (-not (Test-Path $logDirectory)) { New-Item -ItemType Directory -Path $logDirectory | Out-Null }
        Start-Process -FilePath $node.Source -ArgumentList 'server.js' -WorkingDirectory $Backend `
            -WindowStyle Minimized `
            -RedirectStandardOutput (Join-Path $logDirectory 'first-setup-out.log') `
            -RedirectStandardError (Join-Path $logDirectory 'first-setup-error.log')
