...
exports.uploadEvidence=async(req,res)=>{let diskPath=null;try{const ticketId=Number(req.params.ticketId);const a=await access(ticketId,req.user);if(!a.ok)return sendError(res,a.code===404?'Ticket not found':'Access denied',{statusCode:a.code,errorCode:'EVIDENCE_ACCESS_DENIED'});if(!req.file)return sendError(res,'No evidence file uploaded',{statusCode:400,errorCode:'VALIDATION_ERROR'});if(!signatureOk(req.file))return sendError(res,'File content does not match the declared file type',{statusCode:400,errorCode:'INVALID_FILE_SIGNATURE'});await fs.mkdir(uploadDir,{recursive:true});const filename=`${crypto.randomUUID()}${mimeExt[req.file.mimetype]}`;diskPath=path.join(uploadDir,filename);await fs.writeFile(diskPath,req.file.buffer,{flag:'wx'});const lat=req.body.gps_lat===''?null:Number(req.body.gps_lat),lng=req.body.gps_lng===''?null:Number(req.body.gps_lng);if((lat!==null&&(!Number.isFinite(lat)||lat<-90||lat>90))||(lng!==null&&(!Number.isFinite(lng)||lng<-180||lng>180))){await fs.unlink(diskPath);diskPath=null;return sendError(res,'GPS coordinates are invalid',{statusCode:400,errorCode:'INVALID_GPS_COORDINATES'});}const [result]=await db.query(`INSERT INTO evidence(ticket_id,file_path,file_name,file_type,file_size,uploaded_by,gps_lat,gps_lng) VALUES(?,?,?,?,?,?,?,?)`,[ticketId,`/uploads/evidence/${filename}`,path.basename(req.file.originalname),req.file.mimetype,req.file.size,req.user.id,lat,lng]);await logAudit({userId:req.user.id,action:'EVIDENCE_UPLOADED',entityType:'evidence',entityId:result.insertId,metadata:{ticketId,fileName:path.basename(req.file.originalname)},req});return sendSuccess(res,'Evidence uploaded successfully',{id:result.insertId,ticketId,fileName:req.file.originalname,filePath:`/uploads/evidence/${filename}`,fileType:req.file.mimetype,fileSize:req.file.size},{statusCode:201});}catch(error){if(diskPath)try{await fs.unlink(diskPath)}catch{}console.error(error);return sendError(res,error.message?.startsWith('Invalid file type')?error.message:'Server error while uploading evidence',{statusCode:error.message?.startsWith('Invalid file type')?400:500,errorCode:'EVIDENCE_UPLOAD_FAILED'});}};
exports.getTicketEvidence=async(req,res)=>{try{const ticketId=Number(req.params.ticketId),a=await access(ticketId,req.user);if(!a.ok)return sendError(res,a.code===404?'Ticket not found':'Access denied',{statusCode:a.code,errorCode:'EVIDENCE_ACCESS_DENIED'});const [items]=await db.query(`SELECT e.*,e.file_type mime_type,u.name uploaded_by_name FROM evidence e LEFT JOIN users u ON e.uploaded_by=u.id WHERE e.ticket_id=? ORDER BY e.created_at DESC`,[ticketId]);return sendSuccess(res,'Ticket evidence fetched successfully',items,{legacy:{evidence:items}});}catch(error){console.error(error);return sendError(res,'Server error while fetching evidence',{statusCode:500,errorCode:'EVIDENCE_FETCH_FAILED'});}};
...
exports.getEvidenceFile = async (req, res) => {
    try {
        ...
        const [rows] = await db.query(
            `SELECT e.id, e.ticket_id, e.file_path, e.file_name, e.file_type, t.user_id
             FROM evidence e
